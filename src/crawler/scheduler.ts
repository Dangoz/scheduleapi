...
module.exports = async () => {
  ...
  schedule.scheduleJob('0 */15 * * * *', async () => {
    require("./jobs/channel-update.job")(youtubeAPI);
  });
  ...
  schedule.scheduleJob('50 */5 * * * *', () => {
    require("./jobs/recent-video-feed.job")()
  });

  // update 20 least updated video, every min, Q1440
  schedule.scheduleJob('5 * * * * *', () => {
    require("./jobs/video-update.job")(youtubeAPI);
  })

  // update upcoming videos, every 30secs, Q2880
  schedule.scheduleJob('*/30 * * * * *', () => {
    require("./jobs/upcoming-update.job")(youtubeAPI);
  });

  // update live videos, every 20secs, Q4320
  schedule.scheduleJob('*/20 * * * * *', () => {
    require("./jobs/live-update.job")(youtubeAPI);
  })
}
